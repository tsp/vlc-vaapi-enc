 ...
 * $Id: 12d42dbd61585e94404afc71022270280bb23271 $
 *
...
void FastCopyPlane(uint8_t *dst, size_t dst_pitch,
                   const uint8_t *src, size_t src_pitch,
                   unsigned width, unsigned height,
                   copy_cache_t *cache)
{
    ...
    if (vlc_CPU_SSE2())
    {
        SSE_CopyPlane(dst, dst_pitch, src, src_pitch,
                      cache->buffer, cache->size,
                      width, height, cpu);
        asm volatile ("emms");
        return;
    ...
    CopyPlane(dst, dst_pitch, src, src_pitch, width, height);
}
